...
class DiffusionModule(nn.Module):
    ...
    def get_loss(self, x0, class_label=None, noise=None):
        ...
        x_t = self.var_scheduler.add_noise(x0, timestep, noise)[0]  # [B, C, H, W]
    ...
    def sample(
        ...
        if do_classifier_free_guidance:
            ...
            # Prepare unconditional (null) and conditional class labels
            null_label = torch.zeros_like(class_label)
            #cfg_class_label = torch.cat([null_label, class_label], dim=0).to(self.device)
            #######################
        ...
        for t in tqdm(self.var_scheduler.timesteps):
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred_uncond = self.network(x_t, timestep=t.to(self.device), class_label=null_label.to(self.device))
                noise_pred_cond = self.network(x_t, timestep=t.to(self.device), class_label=class_label.to(self.device))
                # CFG interpolation
