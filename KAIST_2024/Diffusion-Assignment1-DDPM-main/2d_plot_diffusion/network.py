...
class SimpleNet(nn.Module):
    def __init__(
        ...
        self.time_linear_input_layer = TimeLinear(self.dim_in, self.dim_hids[0], self.num_timesteps)
        self.time_linear_hidden_layers = nn.ModuleList([
            TimeLinear(self.dim_hids[i], self.dim_hids[i + 1], self.num_timesteps) for i in range(len(self.dim_hids) - 1)
        ])
        self.time_linear_output_layer = TimeLinear(self.dim_hids[-1], self.dim_out, self.num_timesteps)
        self.relu = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        ######## TODO ########
        # DO NOT change the code outside this part.
        x = self.relu(self.time_linear_input_layer(x, t))
        for layers in self.time_linear_hidden_layers:
                x = self.relu(layers(x, t))
        x = self.time_linear_output_layer(x,t)
        ######################
        return x
