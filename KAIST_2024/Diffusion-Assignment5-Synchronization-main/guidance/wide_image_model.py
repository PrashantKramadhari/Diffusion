...
class WideImageModel(BaseModel):
    ...
    def forward_mapping(self, z_t, **kwargs):
        ...
        for h_start, h_end, w_start, w_end in self.mapper:
            # Extract the patch from the canonical latent z_t
            xt = z_t[:, :, h_start:h_end, w_start:w_end]
            xts.append(xt)
        
        # Stack all patches along the batch dimension
    ...
    def inverse_mapping(self, x_ts, **kwargs):
        ...
        # Reset accumulation tensors
        self.count.zero_()
        self.value.zero_()
        
        # Accumulate patches back to canonical space
        for i, (h_start, h_end, w_start, w_end) in enumerate(self.mapper):
            # Add the i-th patch to the canonical latent at the corresponding position
